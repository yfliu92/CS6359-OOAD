      ...
      <div class="row">
       <%@ include file = "./app/shared/sidebar/sidebar.jsp" %>
        <div class="col-md-10 page-content-container">
			...
			<br>Year: <input type="text" name="year" value=<%=course.getYear() %> onchange="changeyear()" style="width:50px">
			<br>Semester: <input type="text" name="semester" value=<%=course.getSemester() %> style="width:70px">
			...
			<br>Days: 
			<input type="checkbox" name="days" value="M" <%if(course.getDays().indexOf("M")!=-1){ %> checked="checked" <%} %> />Monday
			<input type="checkbox" name="days" value="Tu" <%if(course.getDays().indexOf("Tu")!=-1){ %> checked="checked" <%} %>  />Tuesday
			<input type="checkbox" name="days" value="W" <%if(course.getDays().indexOf("W")!=-1){ %> checked="checked" <%} %> />Wednesday
			<input type="checkbox" name="days" value="Th" <%if(course.getDays().indexOf("Th")!=-1){ %> checked="checked" <%} %> />Thursday
			<input type="checkbox" name="days" value="F" <%if(course.getDays().indexOf("F")!=-1){ %> checked="checked" <%} %> />Friday
			<input type="checkbox" name="days" value="Sa" <%if(course.getDays().indexOf("Sa")!=-1){ %> checked="checked" <%} %> />Saturday
			<input type="checkbox" name="days" value="Su" <%if(course.getDays().indexOf("Su")!=-1){ %> checked="checked" <%} %> />Sunday
			...
			<% for(User u:list){%>
			<option value= <%=u.getId()%> <%if(course.getTeacher_id().equals(u.getId())){%> selected="selected" <%} %>><%=u.getF_name()+" "+u.getL_name() %></option>
			<%} %>
			</select>
