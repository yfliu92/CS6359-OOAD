...
function getPrefix() {
    ...
    if (prefix.length <= 1) {
        ...
        for (var [key, value] of myMap) {
            prefix[i + 1] = new Option(value, key);
            i++;
        }
    }
}
...
/*
 * initial values when edit course
 * */
function getValues(pre, sday, eday) {
    var prefix = document.form1.prefix;
    var i = 0;
    for (var [key, value] of myMap) {
        prefix[i + 1] = new Option(value, key);
        if (key == pre) prefix[i + 1].selected = true;
        i++;
    }
    ...
    for (var [key, value] of mmap) {
        smonth[j + 1] = new Option(value, key);
        if (sm == key) smonth[j + 1].selected = true;
        emonth[j + 1] = new Option(value, key);
        if (em == key) emonth[j + 1].selected = true;
        j++;
    }

    var year = document.form1.year.value;
    var m1 = new Set(["01", "03", "05", "07", "08", "10", "12"]);
    var m2 = new Set(["04", "06", "09", "11"]);
    var day1 = document.form1.sday;
    if (m1.has(sm)) {
        for (var i = 1; i <= 31; i++) {
            day1[i] = new Option(i, i);
            if (sd == i) day1[i].selected = true;
        }
    }
    else if (m2.has(sm)) {
        for (var i = 1; i <= 30; i++) {
    ...
    var day2 = document.form1.eday;

    if (m1.has(em)) {
        for (var i = 1; i <= 31; i++) {
            day2[i] = new Option(i, i);
            if (ed == i) day2[i].selected = true;
        }
    }
    else if (m2.has(em)) {
        for (var i = 1; i <= 30; i++) {
...
function changeyear(x) {
    var year = document.form1.year.value;
    document.form1.syear.value = year;
...
function getMonth(x) {
    ...
    if (x == 1) {
        ...
        if (smonth.length <= 1) {
            var i = 0;
            for (var [key, value] of mmap) {
                smonth[i + 1] = new Option(value, key);
                i++;
            }
        }
    }
        ...
        if (emonth.length <= 1) {
            var i = 0;
            for (var [key, value] of mmap) {
                emonth[i + 1] = new Option(value, key);
                i++;
            }
        }
    }
...
function getDay(x) {
    var m1 = new Set(["1", "3", "5", "7", "8", "10", "12"]);
    var m2 = new Set(["4", "6", "9", "11"]);
    var year;
    var month;
    var day;
    if (x == 1) {
...
function isValid() {
    if (document.form1.prefix.value == 0) {
        alert("Please choose a prefix for the course!");
        return false;
    }
}
...
function edit(button) {
    var index = -1;
    for (var i = 0; i < cform.ebut.length; i++) {
        if (cform.ebut[i] == button) {
            index = i;
            break;
        }
    }
